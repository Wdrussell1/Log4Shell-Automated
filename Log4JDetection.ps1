...
[io.compression.zipfile]::ExtractToDirectory("$PSScriptRoot\yara32.zip", "$PSScriptRoot\")
[io.compression.zipfile]::ExtractToDirectory("$PSScriptRoot\yara64.zip", "$PSScriptRoot\")

#Logging Setup
If(!(test-path $LogTo))
{
      New-Item -ItemType Directory -Force -Path $LogTo
}
...
$arrFiles | ? {$_ -match '\.jar$'} | % {
    if (select-string -Quiet -Path $_ "JndiLookup.class") {
        ...
        if (!(test-path "$LogTo\L4Jdetections.txt" -ErrorAction SilentlyContinue)) {set-content -path "$LogTo\L4Jdetections.txt" -Value "! CAUTION !`r`n$(get-date)"}
        Add-Content "$LogTo\L4Jdetections.txt" -Value "POTENTIALLY VULNERABLE JAR: $($_)"
...
foreach ($file in $arrFiles) {
    if ($file -match 'log4j' -or $file -match 'L4Jdetections\.txt') {
        ...
        if ($yaResult) {
            ...
            write-host $yaResult
            #write to a file
            if (!(test-path "$LogTo\L4Jdetections.txt" -ErrorAction SilentlyContinue)) {set-content -path "$LogTo\L4Jdetections.txt" -Value "! INFECTION DETECTION !`r`n$(get-date)"}
            Add-Content "$LogTo\L4Jdetections.txt" -Value $yaResult
        }
    }
}
...
#Output of Positive or Negative results
...
if ($script:varDetection -eq 1) {
    ...
    write-host "  $LogTo\L4Jdetections.txt"
